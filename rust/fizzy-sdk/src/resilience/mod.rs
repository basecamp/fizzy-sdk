//! Three ways to stop a struggling Fizzy from taking the caller down with it: a circuit
...
//! and operation as the model names them, `Boards.ListBoards` — so a failing search does not
...
//!
//! ```no_run
//! use fizzy_sdk::resilience::{CircuitBreakerConfig, ResilienceConfig};
//! use fizzy_sdk::{Client, Config, StaticTokenProvider};
//!
//! # fn build() -> Result<Client, fizzy_sdk::Error> {
...
/// does anything Fizzy answered for itself, however unwelcome. What is left is Fizzy failing to
...
impl ResilienceHooks {
    /// The layers the config asks for, in front of `inner`.
...
impl Hooks for ResilienceHooks {
    ...
    /// Fizzy asking for a wait is worth more than the limiter's own accounting, so a 429 arms
...
mod tests {
    ...
    #[test]
    fn only_what_fizzy_failed_to_answer_trips_the_breaker() {
        let cases = [
    ...
    async fn a_breaker_only_config_refuses_once_the_scope_has_failed_enough() {
        ...
        fail(&hooks, &operation("Boards", "ListBoards")).await;
        assert!(
            hooks
                .on_operation_gate(&operation("Boards", "ListBoards"))
                .await
                .is_ok()
        );
        fail(&hooks, &operation("Boards", "ListBoards")).await;
        ...
        let refused = hooks
            ...
            .await
            .unwrap_err();
        assert_eq!(Some(Refusal::CircuitOpen), refused.refusal());
        assert_eq!("circuit breaker is open", refused.message());
        assert!(
            hooks
                .on_operation_gate(&operation("Boards", "GetBoard"))
                .await
                .is_ok()
        );
    }
    ...
    async fn a_bulkhead_with_no_wait_refuses_a_scope_that_is_already_busy() {
        ...
        let op = operation("Boards", "ListBoards");
        ...
        assert_eq!(Some(Refusal::BulkheadFull), refused.refusal());
    ...
    fn operation(service: &'static str, operation: &'static str) -> OperationInfo {
        OperationInfo {
            ...
            resource_type: Cow::Borrowed("board"),
            is_mutation: false,
            resource_id: None,
        }
    }
