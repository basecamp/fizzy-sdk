...
/// The budget a client holds itself to, and whether it takes Fizzy's word over its own.
///
...
pub struct RateLimitConfig {
    ...
    pub burst_size: u32,
    /// Whether a `Retry-After` from Fizzy holds the limiter back until it has passed.
...
/// A token bucket, and whatever wait Fizzy last asked for.
///
...
impl RateLimiter {
    ...
    /// and `None` when the limiter will not have one within the second — or when Fizzy has
    ...
    /// extended, never cut short, so the longest thing Fizzy asked for is what is honoured.
    ...
    /// Whether Fizzy's wait is still in force, forgetting one that has passed on the way.
...
mod tests {
    ...
    #[test]
    fn a_wait_fizzy_asked_for_holds_everything_back_until_it_has_passed() {
        let (clock, now) = test_clock();
        let limiter = RateLimiter::new(RateLimitConfig {
    ...
    #[test]
    fn a_reservation_is_refused_while_fizzy_has_asked_for_a_wait() {
        let (clock, _now) = test_clock();
        let limiter = RateLimiter::new(RateLimitConfig {
            clock,
            ..RateLimitConfig::default()
