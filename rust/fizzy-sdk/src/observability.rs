...
/// free to report itself as something other than the route it sends — a wrapper that
/// signs in through a magic link says `MagicLinkLogin`.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct OperationInfo {
    /// The service handle the call came from: `Boards`, `AccessTokens`.
    pub service: Cow<'static, str>,
    /// The operation as the model names it: `ListBoards`, `MoveCard`.
    pub operation: Cow<'static, str>,
    /// The kind of record the call acts on, snake_cased: `board`, `card`.
    pub resource_type: Cow<'static, str>,
    /// The call changes something.
...
pub struct RequestResult<'a> {
    /// What Fizzy answered, or `None` when nothing came back at all.
    pub status: Option<StatusCode>,
    /// How long Fizzy took to answer, before the body was read.
    pub duration: Duration,
    /// What this request failed with, whether or not the SDK went on to resend it.
    pub error: Option<&'a Error>,
    /// The body came out of the response cache: Fizzy answered 304 and the SDK read the
...
///
/// use fizzy_sdk::Error;
/// use fizzy_sdk::observability::{Hooks, OperationInfo, OperationState};
///
/// struct Timing;
///
...
pub trait Hooks: Send + Sync {
    ...
    /// answer, so a policy can refuse a call before it reaches Fizzy.
    ///
    ...
    /// Told an operation is starting. Whatever comes back is handed to
    /// [`Hooks::on_operation_end`].
    fn on_operation_start(&self, _op: &OperationInfo) -> OperationState {
        None
    }
    ...
    /// Told a request is about to go out.
    fn on_request_start(&self, _info: &RequestInfo) {}

    /// Told how a request turned out, whether or not it is resent.
...
mod tests {
    ...
    fn request_info() -> RequestInfo {
        RequestInfo {
            method: Method::GET,
            url: Url::parse("https://fizzy.example/999/boards.json").unwrap(),
            attempt: 1,
        }
    }
