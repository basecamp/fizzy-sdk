...
/// A response the cache holds for a URL: the validator Fizzy sent with it, and the body it
/// validates.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct CachedResponse {
    /// The `ETag` the server sent.
    pub etag: String,
    /// The body it validates.
    pub body: Bytes,
}
...
pub trait ResponseCache: Send + Sync {
    ...
    /// Holds a response under the key.
...
impl FileCache {
    /// A cache over a directory, created on first write.
...
impl ResponseCache for FileCache {
    ...
    /// shared with the Fizzy CLI, which keeps its credentials and its own state alongside, so
