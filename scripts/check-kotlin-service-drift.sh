...
openapi_ops=$(jq -r '[.paths | to_entries[] | .value | to_entries[] | select(.key != "parameters") | .value.operationId] | sort[]' "$OPENAPI")

# Extract operation strings from generated Kotlin service files
kt_ops=$(grep -rohE 'operation = "[^"]*"' "$KT_SERVICES"/*.kt 2>/dev/null | sed 's/operation = "\(.*\)"/\1/' | sort -u)
