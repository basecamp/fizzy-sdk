...
- **Rust**: `Client` is `Clone + Send + Sync` and shares one connection pool and one set of hooks across clones. Resilience and cache state live behind `Mutex`es. The credential the client holds, and every generated response field that carries a secret — a session token, an access token's `token`, a webhook's `signing_secret` — is a `SensitiveString`: its `Debug` output is redacted, and the value is read only through `expose()`. `#![forbid(unsafe_code)]` holds for the whole crate.
