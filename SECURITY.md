...
- **Rust**: `Client` is `Clone + Send + Sync` and shares one connection pool and one set of hooks across clones. Resilience and cache state live behind `Mutex`es. The credential the client holds is a `SensitiveString` whose `Debug` output is redacted; the generated response fields that carry a secret — a session token, an access token's `token`, a webhook's `signing_secret` — are plain strings the application must treat as such, and nothing in the crate logs them. `#![forbid(unsafe_code)]` holds for the whole crate.
