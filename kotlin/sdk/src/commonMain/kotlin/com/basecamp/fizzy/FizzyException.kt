...
/**
 * Sealed class hierarchy for Fizzy API errors.
 *
 * Enables exhaustive `when` matching for error handling:
 * ```kotlin
 * try {
 *     account.cards.get(cardNumber)
 * } catch (e: FizzyException) {
 *     when (e) {
 *         is FizzyException.Auth -> println("Token expired")
 *         is FizzyException.NotFound -> println("Not found")
 *         is FizzyException.RateLimit -> println("Retry in ${e.retryAfterSeconds}s")
 *         is FizzyException.Forbidden -> println("Access denied")
 *         is FizzyException.Validation -> println("Invalid input: ${e.message}")
 *         is FizzyException.Network -> println("Network error")
 *         is FizzyException.Api -> println("Server error: ${e.httpStatus}")
 *         is FizzyException.Ambiguous -> println("Ambiguous match")
 *         is FizzyException.Usage -> println("Bad arguments: ${e.message}")
 *     }
 * }
 * ```
 */
sealed class FizzyException(
    message: String,
    ...
    class Auth(
        ...
        hint: String? = "Check your access token or session cookie",
        requestId: String? = null,
        cause: Throwable? = null,
    ) : FizzyException(message, CODE_AUTH, hint, 401, false, requestId, cause)
    ...
    class Forbidden(
        ...
        requestId: String? = null,
        cause: Throwable? = null,
    ) : FizzyException(message, CODE_FORBIDDEN, hint, 403, false, requestId, cause)
    ...
    class NotFound(
        ...
        requestId: String? = null,
        cause: Throwable? = null,
    ) : FizzyException(message, CODE_NOT_FOUND, hint, 404, false, requestId, cause)
    ...
    class RateLimit(
        ...
        requestId: String? = null,
        cause: Throwable? = null,
    ) : FizzyException(message, CODE_RATE_LIMIT, hint, 429, true, requestId, cause)
    ...
    class Network(
        ...
        cause: Throwable? = null,
    ) : FizzyException(message, CODE_NETWORK, hint, null, true, null, cause)
    ...
    class Api(
        ...
        requestId: String? = null,
        cause: Throwable? = null,
    ) : FizzyException(message, CODE_API, hint, httpStatus, retryable, requestId, cause)
    ...
    class Validation(
        ...
        httpStatus: Int = 422,
        requestId: String? = null,
    ) : FizzyException(message, CODE_VALIDATION, hint, httpStatus, false, requestId)
    ...
    ) : FizzyException("Ambiguous $resource", CODE_AMBIGUOUS, hint)

    /** Usage error (bad arguments, configuration errors). */
    class Usage(
        message: String,
        hint: String? = null,
    ) : FizzyException(message, CODE_USAGE, hint)
    ...
    companion object {
        ...
        /** Creates a [FizzyException] from an HTTP status code and response body. */
        fun fromHttpStatus(
            httpStatus: Int,
            message: String? = null,
            hint: String? = null,
